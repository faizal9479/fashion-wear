...
router.post("/add-to-cart", (req, res) => {
  if (req.session.userLoggedIn) {
    ...
    userHelpers
      .addToCart(req.body.id, req.session.user._id, req.body.count)
      .then(() => {
        res.json({ status: true });
      });
  } else if (req.session.user == undefined) {
...
router.post("/place-order", async (req, res) => {
  ...
  userHelpers
    .placeOrder(req.body, products, totalPrice, offerPrice)
    .then((orderId) => {
      if (req.body["payment-method"] == "COD") {
        req.session.user.orderId = orderId;
        db.get()
          .collection(collection.CART_COLLECTION)
          .removeOne({ user: objectId(req.session.user._id) }); //removeing cart
...
router.get("/confirm-order", verifyLogin, (req, res) => {
  if (req.session.user.orderId == null) {
    ...
    userHelpers.getSingleOrder(req.session.user.orderId).then(async (order) => {
      let products = await userHelpers.getOrderProducts(
        req.session.user.orderId
      );

      res.render("user/confirm-order", {
...
router.post("/coupon", (req, res) => {
  ...
  userHelpers
    .VerifyCoupon(couponCode, req.session.user._id)
    .then(async (details) => {
